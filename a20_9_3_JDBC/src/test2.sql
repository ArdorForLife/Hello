...
update member set pwd='1234' where name='장미';
select * from member;
update member set code='1005' where name like '홍%';
update member set age=age+1 where name='홍길동' and pwd='444';

--DB에서 레코드삭제(제거) delete from 테이블명 where 조건;
delete from member where name='장미';
select * from member;
...
--구조변경 alter table 테이블명 add(   )   추가
--구조변경 alter table 테이블명 modify     구조변경
--구조변경 alter table 테이블명 drop       컬럼 제거
